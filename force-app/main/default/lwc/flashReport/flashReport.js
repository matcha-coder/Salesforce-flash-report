...
import saveFlashReport from '@salesforce/apex/FlashReportController.saveFlashReport';
import getFlashReport from '@salesforce/apex/FlashReportController.getFlashReport';
...
export default class FlashReport extends LightningElement {
    ...
    async loadSavedState() {
        try {
            const report = await getFlashReport({ recordId: this.recordId });
            if (report.Selected_Object__c) {
                this.selectedObject = report.Selected_Object__c;
            }
            if (report.Selected_Fields__c) {
                this.selectedFields = report.Selected_Fields__c.split(',').map(f => f.trim());
            }
            if (report.Where_Clause__c) {
                this.whereClause = report.Where_Clause__c;
            }
            if (report.Order_By__c) {
                this.orderByField = report.Order_By__c;
            }
            if (report.Limit_Value__c != null) {
                this.limitValue = report.Limit_Value__c;
            }
            await this.loadFields();
            this.buildQuery();
        } catch (error) {
            // No saved state or error loading — use defaults
            console.warn('Could not load saved Flash Report state:', error);
        }
    }

    async handleSaveQuery() {
        if (!this.recordId) {
            this.showError('Save Error', 'No record context found. Place this component on a Flash Report record page.');
            return;
        }
        try {
            await saveFlashReport({
                recordId: this.recordId,
                selectedObject: this.selectedObject,
                selectedFields: this.selectedFields.join(', '),
                whereClause: this.whereClause || '',
                orderBy: this.orderByField || '',
                limitValue: this.limitValue || null,
                soqlQuery: this.queryInput
            });
            this.showSuccess('Query saved to Flash Report record');
        } catch (error) {
            this.showError('Save Error', error.body ? error.body.message : error.message);
        }
    }
